...
### 5. QoS profile has little impact; RMW choice dominates transport latency

The QoS heatmap varied 4 profiles (reliable/best-effort × depth 10/100) across both RMWs.
Transport-level `/image` p99 is determined almost entirely by RMW choice, not QoS:

| QoS Profile | Cyclone DDS p99 | Fast DDS p99 |
|---|---|---|
| best_effort_depth_10 | 0.702 ms | 0.978 ms |
| best_effort_depth_100 | 0.866 ms | 0.979 ms |
| reliable_depth_10 | **0.583 ms** | 0.843 ms |
| reliable_depth_100 | 0.670 ms | 0.834 ms |

Cyclone DDS is 25–40% faster on transport across every QoS combination. The
best overall configuration is **Cyclone DDS + reliable depth 10** (0.583ms p99).

Processing latency (Gaussian blur) is unaffected by QoS — the ~0.3ms variation
across cells is CPU scheduling noise. Notably, Fast DDS with
`best_effort_depth_100` shows an anomalous spike to 7.76ms p99 (e2e),
suggesting large best-effort queues can trigger internal contention in Fast DDS.

**Takeaway:** If you can only tune one variable, pick Cyclone DDS over Fast DDS.
QoS tuning (reliable vs best-effort, depth 10 vs 100) is a second-order concern.
...
- **QoS variation (section 5):** 4 profiles tested. The current heatmap only covers two RMWs; broader RMW coverage would generalize the conclusion.
